...
const upload = multer({ storage: storage }).single('planetImage');
...
exports.uploadPlanetImage = async (req, res) => {
  ...
  upload(req, res, async function(err) {
    try {
      ...
      const result = await client.query('UPDATE planets SET image = $1 WHERE id = $2 RETURNING *', [imagePath, planetId]);
